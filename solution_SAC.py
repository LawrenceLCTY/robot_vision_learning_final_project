...
class Solution(SolutionBase):
    ...
    def init(self, env: FinalEnv, weight_path="SAPIEN_weights.pth"):

        self.agent = SAC_agent(env)
        if os.path.exists(weight_path):
            self.agent.load_weight(weight_path)
        ...
        # idea: scoop 3 times to get as many boxes in the spade as possible
        self.num_scoops = 0
    ...
    def act(self, env: FinalEnv, current_timestep: int):
        ...
        if self.phase == 4:
            left_spade_boxes, right_spade_boxes = self.get_box_on_spade(env)
            if self.num_scoops == 2:
                # new environment
                self.agent.update_box_on_spade(left_spade_boxes, right_spade_boxes)
                current_episode = self.agent.run(env, is_train=True)
        ...
        if self.phase == 5:
            if self.is_spade_above_bin(r2, self.target_bin_position):
        ...
        #
        #     if self.phase == 4:
        #     # this is original hard code to get the target bin
        #     self.counter += 1
        ...
        if self.phase == 6:  # End phase: either continue simulation or end scene
            print('Phase 6')
            self.phase = 0
    ...
    def get_box_on_spade(self, env:FinalEnv):
        box_id_on_left_spade = []
        box_id_on_right_spade = []

        r1, r2, c1, c2, c3, c4 = env.get_agents()
        left_end_effector_pos = r1.get_observation()[2][9].p
        right_end_effector_pos = r2.get_observation()[2][9].p

        box_positions = self.get_box_positions(c4)

        for box_id, position in box_positions.items():
                left_dist = 0
                right_dist = 0
                for i in range(3):
                    left_dist += (position[i] - left_end_effector_pos[i]) ** 2
                    right_dist += (position[i] - right_end_effector_pos[i]) ** 2


                if right_dist < 0.02:
                    box_id_on_right_spade.append(box_id)
                    continue

                if left_dist < 0.02:
                    box_id_on_left_spade.append(box_id)



        return box_id_on_left_spade, box_id_on_right_spade
    ...
    def pick_box(self, c):
        ...
        for i in self.box_ids:
            ...
            if len(m[0]):
                ...
                # print(f"test id {i} segmentation: {m}\n global osition {global_position}")
    ...
    def rotate_spade(self, robot, initial_qpos, rotation_step, total_steps, clockwise=True):
        ...
        last_joint_index = robot.dof - 1
        qpos[last_joint_index] = initial_qpos[last_joint_index] + direction * (2 * np.pi) * (
                    rotation_step / total_steps)
