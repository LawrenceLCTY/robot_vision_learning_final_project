...
class Solution(SolutionBase):
    ...
    def act(self, env: FinalEnv, current_timestep: int):
        ...
        if self.phase == 1:
            ...
            action = np.concatenate([target_pose_left.p, target_pose_right.p])
        ...
        if self.phase == 2:
            ...
            action = np.concatenate([self.pose_left.p, self.pose_right.p])
        ...
        if self.phase == 3:
            ...
            if self.counter < 5000 / 5:
                ...
                action = np.concatenate([t1[:3], self.pose_right.p])
                r1.set_action(t1, [0] * 7, pf_left)
                self.diff_drive(r2, 9, self.pose_right)
            else:
                self.phase = 4
        ...
        if self.phase == 4:
            ...
            if (self.counter < 7000 / 5):
                ...
                # action = np.concatenate([r1.get_observation()[2][9].p, p])
                ...
                # action = np.concatenate([p, r2.get_observation()[2][9].p])

                self.jacobian_drive(r2, 9, Pose(p, q))
            else:
                print("phase 4d")
                # Initialize rotation state if it doesn't exist
                action = np.concatenate([r1.get_observation()[2][9].p, r2.get_observation()[2][9].p])
        ...
        if len(action) == 6 and  self.phase == 4 :
            self.agent.update_tajectory(env, action)
